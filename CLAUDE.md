...
名前の由来（野球の「オープン戦」にかけて）:
- **オープン宣伝** - プロモーション状況をオープンに晒す
- **オープン戦（戦い）** - 有名になれるかの戦いの始まり。開発者がドキドキしながら世に送り出す序盤戦

プロモーション状況をオープンに晒して、自虐も自慢もできるツール。

## コンセプト

- 投稿先（Zenn, Qiita, Note, X, Reddit, GitHub）を登録
...
| サービス | 方法 | 取得データ |
|----------|------|-----------|
| GitHub | API（認証なし可） | Star数、Issue数 |
| Reddit | API（OAuth） | upvote数、コメント数 |
| Zenn | ページスクレイピング | いいね数、コメント数 |
| Qiita | API（認証なし可） | いいね数、ストック数、コメント数 |
| Note | ページスクレイピング | スキ数、コメント数 |
| X | 手動入力（API厳しい） | いいね数、RT数、リプライ数 |
